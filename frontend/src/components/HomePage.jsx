...
import {
  ...
  addChannels,
} from "../services/channelsSlice.js";
import {
  selectors as messagesSelectors,
  addMessages,
} from "../services/messagesSlice.js";
...
const HomePage = () => {
  ...
  const getVariant = (channelId) =>
    channelId === currentChannelId ? "secondary" : "";
  ...
  const renderChannels = () => {
    ...
    return channels.map((channel) => (
      <Nav.Item
        key={channel.id}
        className="w-100"
        onClick={handleSetCurrentChannelId(channel.id)}
      >
        {channel.removable
          ? renderRemovableChannel(channel)
          : renderUnremovableChannel(channel)}
      </Nav.Item>
    ));
  };
